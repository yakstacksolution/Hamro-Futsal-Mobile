...
class VenueStatusWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // No BackdropFilter: the pill always sits on an opaque card surface and is
    // itself 85% opaque, so blurring what is behind it changed nothing on
    // screen — yet it cost an offscreen blur pass per card on every scroll
    // frame. The decoration's own radius does the rounding, so no clip either.
    return Container(
